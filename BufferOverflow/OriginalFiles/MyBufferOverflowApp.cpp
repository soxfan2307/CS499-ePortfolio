...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        //Detecting the overflow; if detected result equals maximum limit for type
        ...
        // If not, continue and add the increment to result
        else 
            result += increment;
    }
...
void test_overflow()
{
    ...
    //result2 equals add_numbers method to check if method fails
    T result2 = add_numbers<T>(start, increment, steps + 1);
    //result calls add_numbers method with no overflow (no extra step) to be able to provide original value prior to method change
    result = add_numbers<T>(start, increment, steps);
    // if result to is equal to the type maximum (verify overflow) then increase result to be able to show original value
    // and print to console the report that Overflow has occured along with the original value
        if (result2 == std::numeric_limits<T>::max()) {
            result += increment;
            std::cout << "Overflow detected! " << +result << std::endl;
        }
        //else continue with result2 as no overflow has occured and value can be printed to console
        else {
...
void test_underflow()
{
    ...
    //result2 equals subtract_numbers method to check if method fails
    T result2 = subtract_numbers<T>(start, decrement, steps + 1);
    //result calls subtract_numbers method with no underflow to be able to provide original value prior to method change
    result = subtract_numbers<T>(start, decrement, steps);
    // if result to is equal to the type minimum (verify underflow) then decrease result to be able to show original value
    // and print to console the notification that underflow was detected along with the original result
    
    if (result2 == std::numeric_limits<T>::min()) 
    {
        result -= decrement;
       std::cout << "Underflow detected! " << +result << std::endl;
    }
    //else continue with result2 as no underflow occured
    else {
